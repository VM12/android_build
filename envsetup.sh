...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^vm12_") ; then
       VM12_BUILD=$(echo -n $1 | sed -e 's/^vm12_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $VM12_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       VM12_BUILD=
    fi
    export VM12_BUILD
...
function breakfast()
{
    target=$1
    local variant=$2
    VM12_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/vm12/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the VM12 model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
            lunch vm12_$target-$variant
        fi
    fi
    return $?
}
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/vm12/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
