...
if not depsonly:
    print"Device %s not found. Attempting to retrieve device repository from VM12 Github (http://github.com/VM12)." % device

repositories = []

page = 1
while not depsonly:
    result = json.loads(urllib2.urlopen("https://api.github.com/users/VM12/repos?page=%d" % page).read())
    if len(result) == 0:
        break
    for res in result:
...
def get_from_manifest(devicename):
    try:
        lm = ElementTree.parse(".repo/local_manifests/vm12_manifest.xml")
        lm = lm.getroot()
    except:
...
def add_to_manifest_dependencies(repositories):
    ...
    for repository in repositories:
        ...
        if existing_project != None:
            ...
            if existing_project.attrib['revision'] == repository['branch']:
                print 'VM12/%s already exists' % (repo_name)
            else:
        ...
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "vm12", "name": repo_name, "revision": "vm12" })
    ...
    f = open('.repo/local_manifests/vm12_manifest.xml', 'w')
    f.write(raw_xml)
    f.close()
    ...
    for repository in repositories:
        ...
        if existing_project != None:
            if existing_project.attrib['revision'] == repository['branch']:
                ...
                print 'updating branch for VM12/%s to %s' % (repo_name, repository['branch'])
                existing_project.set('revision', repository['branch'])
            continue

        print 'Adding dependency: VM12/%s -> %s' % (repo_name, repo_target)
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "vm12", "name": "VM12/%s" % repo_name, "revision": "vm12" })
...
def fetch_dependencies(repo_path):
    ...
    dependencies_path = repo_path + '/vm12.dependencies'
    syncable_repos = []
...
if depsonly:
    ...
    for repository in repositories:
        ...
        if repo_name.startswith("device_") and repo_name.endswith("_" + device):
            ...
            add_to_manifest([{'repository':repo_name,'target_path':repo_path,'branch':'vm12'}])
...
print "Repository for %s not found in the VM12 Github repository list. If this is in error, you may need to manually add it to .repo/local_manifests/vm12_manifest.xml" % device
